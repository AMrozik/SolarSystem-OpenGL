...
const char* kEarthTextureFile="neptunemap.tga";
const char* kMoonTextureFile="proteus.tga";
const char* kSunTextureFile="sun2.tga";
const char* kSkyTextureFile="nightsky.tga";
...
Window::Window(const char * title, int width, int height){
    ...
    height_ = height;
    last_time_ = 0;
    kula_.SetInitAngle(15);
    kula_.SetVelocity(-45);
...
void Window::InitModels(){
    kula_.Initialize(kTorusN, kTorusM, kTorusR, kTorusr);
    kula_.SetTexture(sun_texture_);
    kula_.SetTexturePrim(earth_texture_);
    kula_.SetTextureBis(moon_texture_);
    kula_.SetTextureUnit(GL_TEXTURE0);
    kula_.SetMaterial(kBlueMaterial);
    kula_.SetMaterialPrim(kYellowMaterial);
    kula_.SetMaterialBis(kYellowMaterial);
...
void Window::InitTextures(){
    ...
    earth_texture_.Initialize(kEarthTextureFile);
    moon_texture_.Initialize(kMoonTextureFile);
...
void Window::Render(){
    ...
    kula_.Move((float)(now - last_time_) / CLOCKS_PER_SEC);
    last_time_ = now;
    
    switch (current_program_){
    case POINT_PROGRAM:
        kula_.Draw(point_program_);
        plane_.Draw(program_);
    break;
    case SUN_PROGRAM:
        kula_.Draw(sun_program_);
        plane_.Draw(program_);
    break;
    case SPOT_PROGRAM:
        kula_.Draw(spot_program_);
        plane_.Draw(program_);
    break;
    }
    glutSwapBuffers();
